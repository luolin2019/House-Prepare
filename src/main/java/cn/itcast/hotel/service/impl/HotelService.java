...
public class HotelService extends ServiceImpl<HotelMapper, Hotel> implements IHotelService {
    ...
    public PageResult search(RequestParams params) {
        try {
            ...
            //SearchSourceBuilder request1 = new SearchSourceBuilder();

            request.source().from((page - 1) * size).size(size);


            if (StringUtils.isNotBlank(location)&&sortBy.equals("default")) {
...
//                        .order(SortOrder.ASC)
            ...
            // 4.解析响应
            return handleResponse(response,sortBy);
        } catch (IOException e) {
    ...
    private PageResult handleResponse(SearchResponse response,String sortBy) {
        ...
        if (sortBy.equals("score")) {
            Collections.sort(hotels, new Comparator<HotelDoc>() {
                @Override
                public int compare(HotelDoc o1, HotelDoc o2) {//链表降序
                    return Double.compare(o2.getScore(), o1.getScore());
                }
            });
        } else if (sortBy.equals("price")) {//按价格从低到高排序
            Collections.sort(hotels, new Comparator<HotelDoc>() {
                @Override
                public int compare(HotelDoc o1, HotelDoc o2) {//链表升序
